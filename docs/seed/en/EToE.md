...
$$æ_s = \arg\min_{æ \in Æ_c} (| \vec{i}_\text{æ} - \sum_{\text{æ}' \in Æ_c} \vec{i}_{\text{æ}'} |)$$

The algorithm for selecting a substantial etheron can be written in an equivalent, most compact form as finding the "etheron index" ($æ_s$) whose weighted direction differs minimally from the sum of the weighted directions of the entire group:

$$\text{æ}_s = \arg\min_{\text{æ} \in Æ_c} (| a_\text{æ}\,\vec{d}_\text{æ} - \sum_{\text{æ}' \in Æ_c} a_{\text{æ}'}\,\vec{d}_{\text{æ}'} |)$$

...
The transition of an etheron from the "rest" state ( $s1_æ$ ) to the "moving from the cell" state ( $s3_æ$ ), provided that there are no other etherons in the "rest" state in the cell, occurs if its position in space ( $\vec{r}$<sub>æ</sub> ) at the current time tick ( $t$<sub>Æ</sub> ) does not coincide with the position $\vec{p}_{\text{æ}}(t_{\text{Æ}})$, where the etheron should be:

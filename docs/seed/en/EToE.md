...
In the simplest model, copying the "position" parameter ( $ \vec{r} _{æ} $ ) of each etheron into the "collision position" parameter ( $ \vec{rc} _{\text{æ}} $ ) of that etheron and zeroing the "collision tick" parameter ( $tc_{æ}$ ) of each etheron at every zero tick of time ( $t_Æ=0$ ) prevents uncertainty in the time elapsed since collisions:
